...
def split_into_chapters(paragraphs):
    ...
    # 最初の【題名】が見つかるまでの文章（あらすじ等）を入れる箱
    current_title = "作品情報・プロローグ" 
    current_body = []

    for p in paragraphs:
        # 「【題名】」で始まる行を見つけたら、そこで本を切り分ける
        if p.startswith("【題名】"):
            
            # ★ ユーザー要件：後書きの処理（安全装置）
            # 万が一「後書き」が太字で題名扱いされてしまっていても、分割せずに今の話にそのまま含める
            if "後書き" in p:
                # 「【題名】」というシステム用の目印だけ消して、テキストとして本文に追加
                current_body.append(p.replace("【題名】", ""))
                continue
            
            # これまで貯めていた「今の話」を、本棚（chapters）にしまう
            if current_body: 
                chapters.append({
                    "title": current_title,
                    "body": current_body
            ...
            # 新しい話のタイトルを設定し、本文用の段落リストを空っぽにリセットする
            current_title = p.replace("【題名】", "").strip()
            current_body = []
        
        else:
            # 題名以外の文章（本文、空白行、普通の後書きテキスト）はすべて今の話の箱に入れる
            current_body.append(p)

    # ループが終わった後、一番最後の話を本棚にしまう
    if current_body:
        chapters.append({
            "title": current_title,
            "body": current_body
...
# Phase 03の分割エンジンを実行
chapters_data = split_into_chapters(final_paragraphs)

# 分割が成功したか、各話のタイトルと段落数をターミナルに出力して確認
print("--- 話数分割レポート ---")
for i, chap in enumerate(chapters_data):
    print(f"[{i}番目のブロック] タイトル: {chap['title']}")
    print(f"  └ 本文のボリューム: {len(chap['body'])} 段落")
    print("-" * 30)