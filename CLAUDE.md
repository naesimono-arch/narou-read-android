...
- コード探索は Grep/Glob 起点（この規模なら十分絞れる。総当たり Read の禁止）。semble はツールの一つ＝キーワードで絞れない意味検索・類似実装探しのときだけ。パス既知なら直接 Read／アーキ全体把握・スキーマ変更は上の必須ゲートが先／編集前の文脈確認は従来どおり Read。
