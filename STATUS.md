...
- 端末DB=**`user_version 13`**（2026-07-10 U1 実機E2Eで **12→13 migration 通過を実測**＝`new_episode_marks` 新設・蔵書9冊全生存・起動即クラッシュ無し・`MigrationTest` 実機 am instrument OK(3 tests)。11→12 は 2026-07-09 (b) Web由来カードの実機確認で通過済み）。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。
