...
- **⚠️ 別ブランチ作業中: `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D 着手／2026-07-03〜）** — main `9c3c500` から分岐。Phase 0-2（垂直スライス＝実機で Kotlin 抽出 1冊通しまで・Chaquopy は残置し revert 可）を実行中。
  ...
  - **次（実機必須＝停止ポイント）**: 実機スパイク(Task 2・穴3検証＝PDFBoxResourceLoader.init が実機で効くか) → 実機フル疎通(Task 9・3PDF→HTML→リーダー目視)。**androidTest は @Before で instrumentation Context により init する**（MainActivity 配線は Phase 3 で）。
