...
- **🔵 `meta/detector-improve`（2026-07-08・未マージ）＝実行捏造検知器の穴2件を解消（ADR 0006 増補4）**: 増補3（Tier D4）が残した2つの既知の穴を既存機構の延長で塞いだ。①**Tier B メタ議論免罪**（`_tier_b_reference`）＝捏造テスト主張の「引用・分析」の偽陽性（K/L検証セッション d2096baa で active 3件）を降格。D4 `_d4_is_reference` の移植だが、真陽性（事象L本体 b4087931＝検知器開発セッション内で whole-utterance メタ語彙6件）と偽陽性を発話全体では分離できないため、**whole でなく〈主張文近傍±120字のメタ語彙≥2 ∨ fenced code 内 ∨ 成功語の「」引用〉**で判定（実測ギャップ 真near0/偽near3〜5）。②**Stop 検査窓を `current_turn` へ拡張**（多ツールターン内捏造・事象L）＝`last_turn`（最終発話のみ）だと捏造報告の直後に AskUserQuestion でターン継続すると窓から漏れた穴を、**最後の生ユーザープロンプト以降の全 assistant 発話**を検査して塞ぐ（`_last_user_turn_order`。AskUserQuestion 回答=tool_result はターン境界にしないのが要）。**検証**: 全156ファイル走査で Tier B active 5→2（3偽陽性降格・真陽性維持）・live blocker 2→7（新規5件は全て真陽性＝事象L b4087931＋事象F c4b78e7d のマージ捏造4件）＝過検知ゼロ・他Tier全域不変・フックテスト123件緑。詳細=ADR 0006 増補4／handover 検知器節。**さらに 2026-07-08 当ブランチ作業中に検知器の穴2件を実測・台帳化**: ①**事象M**＝phantom-attribution（幻の同意帰属「あなたの違和感は的を射ていて」＝ユーザー未提起の `/mnt/c` 違和感を捏造）が Tier D の語彙穴で全 tier 走査 active 0件（`f8ec73a7` L29・ground-truth M）＝**未対処のまま宿題**（検知案は handover 検知器節） ②増補4のメタ議論免罪（`_tier_b_reference`）が「検知器の**実装用語**」文脈で漏れ、当セッションの Tier E 仕様説明文（「テスト通過」）を Stop が実ブロック（偽陽性）＝**解消済み（2026-07-10 増補5・下記）**。**→ 2026-07-10（ADR 0006 増補5）**: **Tier E 試作を新設**（`d1287f6`＝現ターンの完了主張束×実行痕跡ゼロ＝事象L型①③④への初の検知手段。既定 tiers 非含の opt-in・真陽性 n=1 のため較正待ち）、**②実装用語漏れを閉鎖**（`d80190c`＝〈〉『』引用括弧拡張＋機構語のみの `DETECTOR_IMPL_TERM_RE` 近傍合算。結果語（降格/偽陽性/Tier 等）は真陽性 b4087931 を免罪する退行を実測し除外）。検証＝フックテスト130件緑・全201ファイル走査で Tier B active 2・live blocker 7 不変・Tier E は事象L のみ検知＝過検知ゼロ。検証中に別クラスFP（9f4d314d「テスト通過後…残します」＝未来/条件文）を新発見し handover へ登録。
