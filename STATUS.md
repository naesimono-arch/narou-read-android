...
- 端末DB=`user_version 10`（2026-07-08 実機スモークで 9→10 通過・蔵書生存確認済み）→**コードは v11**（2026-07-08 `handover/task-sweep`＝F-G 恒久策で `books.contentSha256` を追加・`MIGRATION_10_11`＝ADD COLUMN のみ）＝次回実機 install で 10→11 migration が走る（**実機通過は未確認**＝handover「実機検証待ち」項参照）。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。
