...
- branch=`main` / HEAD=`75de07c`（2026-07-06 に溜まったブランチを --no-ff で統合＝`feat/exec-fabrication-detector`＝実行捏造検知フック `30e4be9` ＋ `kotlin`＝Chaquopy→PDFBox 移植 `75de07c`。統合後にローカルの `cleanup-pre-uidesign`/`ui-n`/`feat/*`/`kotlin` は削除。未push＝origin より ahead 75。統合前の直近実体は 2026-07-02 の `cleanup-pre-uidesign` 統合バッチ＝ルビ#1修正・UI god file分割・設定シート磨き・本文余白設定化）
- **✅ `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D）を main へ統合済み（2026-07-06・--no-ff マージ `75de07c`）。** main `9c3c500` から分岐し **Phase 1＋垂直スライス＋Phase 3（配線＋実機検証）＋Phase 4（精度回帰ゲート）＋Phase 5（Chaquopy 完全撤去）まで完了＝移植の全フェーズ完了**（2026-07-05）。**Chaquopy/Python は撤去済み＝`git revert` での即復旧は不可（git 履歴からの復元は可能）**。統合時の検証＝マージ済みツリーで `testDebugUnitTest` 全緑（BUILD SUCCESSFUL）を確認。
  ...
  - **→ 移植の全フェーズ（Phase 1〜5）完了・`main` へ統合済み（2026-07-06）。** 一次情報 plan `.claude/plans/kotrin-branch-python-kotrin-graceful-flute-archived-2026-07-06.md` 参照（2026-07-06 にホーム側からリポジトリへ収蔵）。
  - **再開手順の正本 = `.claude/plans/kotrin-branch-python-kotrin-graceful-flute-archived-2026-07-06.md` の「実行ログ & 別セッション引き継ぎ」**（残タスク詳細・WSLビルドコマンド・環境メモ）。腐りにくい知見は `task_diary.md` #30-32。実機 `192.168.1.210:5555` 接続済み（切れたら `adb-bridge`）。
- **✅ `feat/processing-resilience`（本ブランチ・2026-07-07）＝handover A の残り2件を実装（JVM＋実機検証済み）**:
  ① **停止ボタンをページ境界の即中断へ再配線**: 処理中の1冊を子 Job（`currentBookJob`）として起動し `ACTION_STOP` が cancel → `addBook` の進捗コールバック内 `ensureActive()` が次のページ境界で中断（ループ Job ごと cancel しないのは cancel〜finally 間の ACTION_START 取りこぼしレース回避）。停止時は ongoing 通知を `STOP_FOREGROUND_REMOVE` で確実に除去、通知文言も「現在のページの処理を終えて停止します」へ。
  ② **A② 強制終了（OEM kill/OOM/onTimeout）時の通知＋再開**: Room **v7→v8** で `pending_jobs` 新設（enqueue で記帳／成否確定で削除＝成功は Room 登録と同じ NonCancellable 内・失敗は classifyError 時／明示停止は全消し／記帳は `pendingJobDispatcher`(並列度1)で直列化＝「追加直後に停止」の記帳復活レース防止）。SAF 選択時に `takePersistableUriPermission` 取得（解放は記帳削除時）。起動時リカバリ `runStartupRecoveryOnce`（MainActivity.onCreate トリガー・プロセス毎1回・Service 非稼働時のみ）＝孤立HTML掃除（books に無い `novels/<id>/` を削除）→ 未完了ジョブ検出 → snackbar 通知＋権限が生きる分を FGS 再投入、権限喪失分は通知して破棄。onTimeout は pending を残し文言を「アプリを開き直すと再開します」へ。
  検証: `testDebugUnitTest` **113件緑**（pending 記帳・孤立HTML掃除のユニットテスト追加）。スキーマ `schemas/…/8.json` 出力済み（DDL は MIGRATION_7_8 と厳密一致を確認）。**実機検証 3/3 合格（2026-07-07・OPPO PGEM10・Claude adb 自律駆動）**: (c) v7→v8 migration＝`user_version 8`・`pending_jobs` 生成・蔵書7冊保持・クラッシュなし ／ (b) 停止即中断＝N6169DZ(6,551p) の本文 load 2% 時点で停止タップ→**2秒以内**にバナー・通知・Service 消滅（旧挙動なら残り~1.5分完走待ち）・pending 全消し・本未登録・書きかけ dir なし ／ (a) 強制終了→再開＝本文 10% 時点で `am force-stop`→pending 残存＋書きかけ `106e29a8` 残存を確認→再起動で snackbar「中断されていた変換 1 件を再開します」＋孤立 dir 掃除＋FGS 再投入→**完走**（books 8冊目「シャングリラ・フロンティア〜」951章・波ダッシュ U+301C 正常・pending 空）。検証副産物: 実機に N6169DZ の変換済み本（af9a93ca）と `/sdcard/Download/N6169DZ.pdf` が残置（正当な蔵書なのでそのまま。不要なら通常削除で可）。
- 端末DB=`user_version 7`（v6→v7 で `books.addedAt`／`progress.lastReadAt` を追加）。**本ブランチのコードは v8**（`pending_jobs` 新設・上記①②）＝次回実機 install で v7→v8 migration が走る。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。
