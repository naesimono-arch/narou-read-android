...
- **直近の統合（このブランチ束ね）**: `reading/vertical-p4`（縦書き章送り P4・実機体感確認済み）・`build/r8-shrink`（release の R8 収縮＝minify+shrinkResources・APK 20.3→7.8MB）・`perf/macrobenchmark`（性能回帰基盤＝起動/本棚スクロール/章送り/大PDF取込の予算 assert・実機実証済み。設計と全実測＝`.claude/plans/macrobenchmark-kickoff-2026-07-17.md`＋`docs/knowledge/coloros-*`／`macrobenchmark-frametiming-scroll-pitfalls.md`）・`hooks/fabrication-detector`（実行捏造検知器 Tier E3「先行実行フレーミング」）・`ui/skin-framework`（UIスキン着せ替え基盤）を main へ統合。縦書き本体（P0〜P3・P5・P2.5）は統合済み＝縦書きはユーザー到達可能（ADR 0020〔連続横スクロール×自前Compose組版〕）。⚠️ **R8 収縮はリリースでしか出ない収縮起因クラッシュがあり実機回帰が別途必須**（本統合は有効化のみ・PDFBox/Retrofit/OkHttp の keep 確認＋リリース実機回帰は未実施）。
- **ゲート（統合後・要実機/リリース回帰）**: `testDebugUnitTest` **596件**（失敗0・JVMゴールデン回帰4件＋DetectedRules 8件を含む・統合後 `gw testDebugUnitTest` で再確認）／`tools/check_design_tokens.py` OK=137/NG=0（＋Spacing lint: 余白スケール7段 {4,8,12,16,24,32,40}＝ADR0014 §C・NG=0・WARN=0）／`:app:lintDebug` 0 errors・28 warnings（ModifierParameter×3 は Compose 規約準拠で解消済み・残る非ブロックは UsableSpace×2＝抽出前空き容量チェックの保守的 `usableSpace` を意図採用）。
