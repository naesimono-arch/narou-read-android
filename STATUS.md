...
- **⚠️ 別ブランチ作業中: `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D 着手／2026-07-03〜）** — main `9c3c500` から分岐。**Phase 1＋垂直スライス＋Phase 3（配線＋実機検証）＋Phase 4（精度回帰ゲート・≤15版クリーンラン取得）完了**（Chaquopy は残置し revert 可）。**次は Phase 5（Chaquopy 完全撤去＝ロールバック不能点）**。
  ...
  - **→ Phase 1＋垂直スライス＋Phase 3（配線＋実機検証）＋Phase 4（精度回帰ゲート・≤15版クリーンラン取得）完了。次は Phase 5（Chaquopy 完全撤去＝ロールバック不能点）。** plan `~/.claude/plans/kotrin-branch-python-kotrin-graceful-flute.md` の「次段階 Phase 3-6」参照。
