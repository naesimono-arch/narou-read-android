...
- **進行中ブランチ**: `reading/vertical-mode`＝縦書きリーディングモード（方針確定＝ADR 0020〔連続横スクロール×自前Compose組版〕・フェーズ/スパイク＝`.claude/plans/vertical-reading-mode.md`・実装待ち。worktree `~/wt/reading-vertical-mode`）／`perf/macrobenchmark`＝性能回帰基盤（コールド起動の**実機初回計測＝median 252.9ms**〔OPPO/Android16・5反復〕＋**起動予算 assert 実装済み**〔median≤350/max≤500ms・`-e enableBudgetAssert true` ゲート・実行スクリプト `tools/run_macrobenchmark.sh`＝除細動ループ同梱〕・残＝assert 経路の実機実走確認と②〜④シナリオ。ColorOS 3重の壁の回避作法＝`docs/knowledge/coloros-uiautomation-shell-pipe-eof-hang.md`・設計と実測＝`.claude/plans/macrobenchmark-kickoff-2026-07-17.md`。worktree `~/wt/perf-macrobenchmark`）。
