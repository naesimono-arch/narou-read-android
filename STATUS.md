...
- **進行中ブランチ**: `reading/vertical-mode`＝縦書きリーディングモード（方針確定＝ADR 0020〔連続横スクロール×自前Compose組版〕・フェーズ/スパイク＝`.claude/plans/vertical-reading-mode.md`・実装待ち。worktree `~/wt/reading-vertical-mode`）／`perf/macrobenchmark`＝性能回帰基盤（`:macrobenchmark` モジュール＋benchmark 別パッケージ化〔実蔵書保護〕＋コールド起動計測まで新設済み・実機初回計測と予算較正待ち。設計＝`.claude/plans/macrobenchmark-kickoff-2026-07-17.md`。worktree `~/wt/perf-macrobenchmark`）。
