...
- 端末DB=**`user_version 11`**（2026-07-08 `verify/device-sweep` 実機スイープで **10→11 migration 通過を実測**＝`contentSha256` 列追加・蔵書8冊/progress4件 全生存・既存行 sha=NULL・起動即クラッシュ無し・`MigrationTest` 実機 am instrument OK(3 tests)）。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。
