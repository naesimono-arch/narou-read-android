...
- **⚠️ 別ブランチ作業中: `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D 着手／2026-07-03〜）** — main `9c3c500` から分岐。Phase 0-2（垂直スライス＝実機で Kotlin 抽出 1冊通しまで・Chaquopy は残置し revert 可）を実行中。
  - **完了（6コミット・全て testDebugUnitTest 緑）**: `0a23d53` PDFBox-Android 依存追加（Chaquopy 併存） / `41c3b24` ParserRules・CharBox 移植 / `eae9892` TextProcessor 移植 / `53825f4` PdfExtractor 移植（`com.tom_roush.pdfbox.*` import 解決を compileDebugKotlin で実証） / `dc7a090` splitIntoChapters 移植 / `cd6470e` ChapterProcessor(HTML版：前後書き整形・ruby・html.escape 忠実移植）。移植先 `android/app/src/main/java/com/novelreader/pdf/`。抽出コア＋章整形まで移植済。
  - **次**: HtmlExporter（バイト等価ゴールデン＝穴1） → PdfExtractionException + PdfBookExtractor facade → 実機スパイク(穴3検証) → 実機フル疎通。
