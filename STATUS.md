...
- テスト: `testDebugUnitTest` **350件緑**（2026-07-11 `ui/vertical-pdf-import` 時点。Robolectric の Compose UI テスト＝ADR 0009 を含む。統合直後時点は 113件）。実機の恒久精度回帰ゲート＝`PdfExtractorDeviceSpikeTest`（N6169DZ 含む3件通過済み）。
- 端末DB=**`user_version 16`**（2026-07-11 読書状態フィルタの実機投入で **14→15→16 migration 通過を実測**＝`labels`/`book_labels` DROP・`web_reading_progress` 生成〔episode-nav 複製 DDL の疎通〕・**蔵書9冊＋進捗5行 全生存**・起動即クラッシュ無し。13→14 以前の通過実績は git log の旧記述参照）。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。**実機レーン専有＝`ui/vertical-pdf-import`（v16 を刻んだ・2026-07-11 ユーザー承認）**: `feat/episode-nav`（コード v15）は**実機投入禁止**のまま合流し、合流でエンティティ集合が変わり v16 の identity hash が再衝突する場合は **v17 の no-op 再スタンプ**で解消する（v9→v10 の前例＝task_diary #39 追補）。
