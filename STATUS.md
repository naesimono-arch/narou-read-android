...
- 端末DB=**`user_version 14`**（2026-07-10 U2 実機投入で **13→14 migration 通過を実測**＝`labels`/`book_labels` 新設・蔵書9冊全生存・起動即クラッシュ無し・`MigrationTest` 実機 am instrument OK(3 tests)＝7→14 チェーン。12→13 は同日 U1 実機E2E・11→12 は 2026-07-09 (b) 実機確認で通過済み）。⚠️ **旧APKへ逆走すると `migration N→N-1 not found` でクラッシュ＝逆走禁止**（古い→新しいの一方向のみ）。**コード側の採番は先へ進んでいる（いずれも実機未投入）**: `feat/episode-nav` が v15（`web_reading_progress` 新設）・`ui/vertical-pdf-import` が v16（ラベル2テーブル DROP。14→15 は episode-nav と同一 DDL の複製でパス繋ぎ＝task_diary #39 の運用）。**次に実機へ入れるレーンの専有判断は handover 参照**。
