...
- **⚠️ 別ブランチ作業中: `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D 着手／2026-07-03〜）** — main `9c3c500` から分岐。Phase 0-2（垂直スライス＝実機で Kotlin 抽出 1冊通しまで・Chaquopy は残置し revert 可）を実行中。
  ...
  - **⚠️ Phase 3 実機未検証（3e が停止ポイントで残）**: N6169DZ(350万字)長編を**前景サービス＋WakeLock 経路**で実機フル疎通し ColorOS の fg_cpu kill を回避できるか（task_diary #37 の宿題）＋切替後の実書取込フローの目視関門が未実施。次段でユーザー同席のうえ `/device-verify` で実行。
  - **→ Phase 1＋垂直スライス＋Phase 3 コード配線 完了。次は Phase 3 実機検証(3e)。以降 Phase 4（精度回帰ゲート常設）・Phase 5（Chaquopy 完全撤去＝ロールバック不能点）。** plan `~/.claude/plans/kotrin-branch-python-kotrin-graceful-flute.md` の「次段階 Phase 3-6」参照。
