...
- **⚠️ 別ブランチ作業中: `kotlin`（Chaquopy→Kotlin+PDFBox 移植・handover D 着手／2026-07-03〜）** — main `9c3c500` から分岐。Phase 0-2（垂直スライス＝実機で Kotlin 抽出 1冊通しまで・Chaquopy は残置し revert 可）を実行中。
  ...
  - **Phase 4 精度回帰ゲート 実装済＝≤15版クリーンラン待ち（2026-07-05）**: `PdfExtractorDeviceSpikeTest` を診断→**恒久精度回帰ゲート**へ昇格（合格ライン: 全PDF title/author/章数=完全一致・短中編 body_sha256=完全一致・長編 N6169DZ は数値許容帯＝厳しめ char±0.05%/ruby±1.5%/para±8/blank±8 ＋章題≤15件不一致）。**N1453LW/N2959KI は実機 PASS 確認済**。**N6169DZ は ≤3版で「章題11件のみ超過・他全 PASS」を実機確認済＝≤15版は論理的に必ず PASS**（11≤15・抽出決定的）だが、**≤15版のクリーン完走ランは未取得**＝素androidTest が ColorOS の fg_cpu kill で数回落ちた（端末メモリ逼迫時・task_diary #37）。再開手順は handover D「[Phase 4 詰め]」。**発見**: N6169DZ 章題ドリフトは実測**11件**（旧記載「1件」は過少）＝全てグリフ写像差（ダッシュFF0D→2212×6・矢印回転×3・アポストロフィ座標順×2）。9件は正規化候補（handover）。次セッションで ≤15版クリーンラン取得（handover 手順）。
  - **→ Phase 1＋垂直スライス＋Phase 3（配線＋実機検証）完了・Phase 4 ゲート実装済（≤15クリーンラン待ち）。次は Phase 4 詰め→Phase 5（Chaquopy 完全撤去＝ロールバック不能点）。** plan `~/.claude/plans/kotrin-branch-python-kotrin-graceful-flute.md` の「次段階 Phase 3-6」参照。
