...
# ------------------------------------------
# 判定用関数（題名かどうかを判定する仕組み）
# ------------------------------------------
def check_is_title(fontname, fontsize):
    # fontname にデータが存在し、かつ "Bold" が含まれ、
    # フォントサイズが許容誤差の範囲内で一致するかを確認する
    if fontname is not None and FONT_MARKER_TITLE in fontname and math.isclose(fontsize, FONT_SIZE_BODY_TITLE, abs_tol=TOLERANCE):
        return True
    else:
        return False