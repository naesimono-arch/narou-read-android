---
name: shiori-tips
description: 栞書影の先端ワンポイント意匠（SHIORI_TIPS）を安全に増補する手順。安モデルで大量生成→機械検証→横断dedup→正本HTML/Kotlin/書影モック/ゴールデンの同期まで。意匠を増やす・量産するとき最初に読む。
triggers:
  - "栞の意匠を増やしたい"
  - "先端ワンポイントを追加"
  - "SHIORI_TIPS に足したい"
...
2. **正本HTML・書影モック同期**: `shiori-tips-D.html` と `bookshelf-shiori-grid-D.html` の TIPS を Kotlin と同順の総数へ（scratchpad の build_shohon.js / build_gridD_sync.js が手本）。`bookshelf-shiori-consistency-D.html`/`palette-D` も TIPS を持つが色/整合が主眼で任意（handover 参照）。
