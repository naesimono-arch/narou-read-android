...
| v18 → v19 | books に栞書影の個体差列 shioriTipIndex（先端種）/ shioriLenFrac（棒長）を追加（MIGRATION_18_19、`ALTER TABLE books ADD COLUMN shioriTipIndex INTEGER` ＋ `ADD COLUMN shioriLenFrac REAL`＝どちらも nullable・DEFAULT 句なし〔未抽選が既定〕）。取込時に真の乱数で1回抽選した値を焼き付けて栞書影を固定・既存行は NULL→描画側で title 由来の決定論値へフォールバック（旧蔵書の見た目は不変）。ncode/contentSha256 と同型の nullable 追加ゆえ PRAGMA 分岐不要。詳細は AppDatabase.kt の why コメント参照 |
| v19 → v20 | books に sourceUri 列を追加（MIGRATION_19_20、`ALTER TABLE books ADD COLUMN sourceUri TEXT`＝取込元PDFの SAF content:// URI・削除機能用。**採番レーン＝feat/delete-source-pdf**。feat/scraping-prep レーンは同一 DDL を複製してパスを繋ぎ自レーンの列追加を v21 へ退避（task_diary #39 の「並列 worktree の version 先取り」運用）＝**マージ統合時に二重定義を一方へ寄せること**）|
| v20 → v21 | books に sourceUrl / sourceSite 列を追加（MIGRATION_20_21、`ALTER TABLE books ADD COLUMN sourceUrl TEXT`＋`sourceSite TEXT`＝Web取込元の作品URLとサイトアダプタキー〔例 "kakuyomu"〕・PDF由来は NULL。sourceUri〔content:// 削除用〕との列名酷似の意味差は近傍コメント参照。nullable・DEFAULT 句なしの定型）|
