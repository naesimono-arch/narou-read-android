...
- **P4 ジェスチャ再配線**（終端オーバースクロール章送り。overscroll バウンドは無効化し未消費デルタを章送りへ）。完了定義: 次章/前章プレビュー→確定が動作し本文スクロールと非衝突。**再訂正（2026-07-17 main取込時）: settleSwipe・引っ張りプレビューは main に実在した（ブランチ分岐後に実装＝一度目の「誤前提」判定こそ誤り。存在しないのは ChapterPeekPanel という名前だけで、覗き実体は隣章プレビューとして実装済み）。P4の本命＝縦書き LazyRow の終端で未消費の横デルタを nestedScroll で捕捉し、既存 dragOffsetPx/settleSwipe へ接続する（横書きは親Box draggable が拾うが、縦書きでは LazyRow が横ドラッグを消費するため終端捕捉が必要）。**
