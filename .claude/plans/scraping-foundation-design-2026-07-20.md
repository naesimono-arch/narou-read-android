...
### サイト齟齬・規約: 裁定方針（2026-07-20 ユーザー確定）
- **あいまいなもの（規約・齟齬）は後でまとめて裁定**する（アルファポリス除外可否・ハーメルンの扱い等はここに含む・今は保留）。
- **漏れがあったサイトは発見次第に追加**でよい（設計ラウンドで全数検証はしない）。
- **初号機＝カクヨム**はこの保留と無関係（OK側筆頭・規約調査も整合）＝先行実装済み（P2）。
- 注記本文（受領済み・アダプタ実装時の要件）:
  - **注1 = Pixiv**: R-18 タグのページは**ログイン状態でないと隠れる**仕様＝アプリ内ブラウザでログインしてもらう必要がある。
    メンバーページを登録する場合もログイン必要。→ Pixiv アダプタは**セッション/Cookie を持つ内部ブラウザ認証**が前提
    （匿名 HTTP GET では R-18 本文に到達不可）。設計時の論点＝Cookie 共有・年齢認証・ログイン導線。
  - **注2 = アルファポリス**: 連続 DL で**制限がかかり DL が止まる**場合がある。**一定時間後にリトライする仕様**にしつつ、
    時間を空けて DL する方が無難。→ アルファポリス（＝規約齟齬は保留だが実装時の技術要件として記録）アダプタは
    Crawl-delay を厚め＋**制限検知→バックオフ・リトライ**を組み込む。
...
### Room 版衝突: 裁定済み（2026-07-20 ユーザー確定）
- **事実（機械確認済み）**: 並列 worktree `feat/delete-source-pdf`（コミット `3782bec`・**main 未マージ**）が
  **v20 を消費済み**＝`books.sourceUri TEXT`（取込元PDFの content:// URI を削除機能用に永続化）を追加。
  私のブランチ（feat/scraping-prep）は現 **v19**。
- **確定①: 統合順＝feat/scraping-prep を後に統合**（`feat/delete-source-pdf` が先着で v20 を保持）。
  → 私のブランチは **v21 へ退避**。`MIGRATION_19_20`（`sourceUri` 追加）を `feat/delete-source-pdf` と**同一内容で複製**して
  マイグレーションパスを繋ぎ、その上に自分の列を `MIGRATION_20_21` で足す（版番号先取り衝突＝task_diary #39 の定石）。
- **確定②: 独立2列**（provenance は統合しない）。`sourceUrl`（Web作品 https:// URL）＋`sourceSite`（アダプタキー）を
  `sourceUri`（削除用 content:// URI）とは別列で追加。列名の酷似は近傍コメントで意味差を明記して混同を防ぐ。
- **P3 の Room 部分（確定版）**: version=21・`BookEntity` に `sourceUrl:String?`/`sourceSite:String?`（ncode と同型 nullable・
  DEFAULT 句不要）・`MIGRATION_19_20`（sourceUri 複製）＋`MIGRATION_20_21`（`ALTER TABLE books ADD COLUMN sourceUrl TEXT` /
  `sourceSite TEXT`）を addMigrations へ配線・`schemas/…/21.json` を git 追加・MigrationTest に 19→20→21 の chain と
  データ保存の SELECT 列追加。着手時 `/db-migration` 必須。**マージ時に `feat/delete-source-pdf` 側の 19_20 と重複しないよう
  統合レビューで確認**（同一 SQL の二重定義になる＝どちらか一方に寄せる）。
