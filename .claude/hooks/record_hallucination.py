...
def main() -> int:
    ...
    if err:
        ...
        # なぜ「何もするな」を注入するか: 記録はフックで完結しており、幻覚直後の Claude に
        # その場の分類・調査をさせない運用（2026-07-11 変更）。注入文面は幻覚直後の Claude が
        # 最初に読む指示なので、スキル本文より先にここで停止を明示する。
        print(f"[/hallucination 自動キャプチャ完了] snapshot={snapshot}（{lines}行時点）を"
              f"台帳の未確定キューへ記載済み。記録はこれで完結＝このターンでは分類・調査・検知器実行を"
              f"始めず、直前の作業に戻ること。検証は事後の明示依頼で行う（行番号確定はスナップショット基準）。")
    return 0
