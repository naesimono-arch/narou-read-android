...
> **【凍結アーカイブ（2026-07-12）】本ファイルへの新規追記は停止**。既存の `#N` 参照（ADR・コード・memory からの多数の参照）は恒久に有効なので、ファイル自体は残す・リナンバーもしない。
> **新規の知見は `docs/knowledge/` に1知見=1ファイル**で置く（連番採番をやめ、二重採番という問題クラスを構造的に消すため。運用は `docs/knowledge/README.md`）。
>
