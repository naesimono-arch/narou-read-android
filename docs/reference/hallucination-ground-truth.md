...
P＝2026-07-07 発生の入力側捏造（K型の再来・**G と同一セッション・同一発話 L78 の別クラス幻覚**）。assistant 自身の text ブロック内に `user[Request interrupted for tool use]`＋幻のユーザー発話（会話率・複数選択への言及要望）をロールマーカー付きで自己生成（幻発話の内容は実 human 入力に0件）。K と同様に暴走 thinking 前兆なし。**D4 較正の slug 全走査（増補6 開発時）が発見した検知器起点事象**で、A3 変種＋D4 の二重で active 捕捉＝K で「発見するが active 化できず」だった較正穴を増補6 が塞いだ後の初の実捕捉。2026-07-16 人間確認で昇格。
Q＝2026-07-11 発生の実行捏造（N の L44 と構造同型の単発「幻の出力待ち参照」）。直前コマンドは出力返却済み・ユーザー割込→「どう？」の新ターンで実行ゼロのまま「出力が返ってこないので、単純なコマンドで確認します」。**Tier E3（増補7）較正の全走査が発見した検知器起点事象**＝E3 が設計狙いどおり N 型の単発再発を捕捉した初例。2026-07-16 人間確認で昇格。
...
## P. 幻のユーザー割込＋発話をロールマーカー付きで自己生成＝G と同一発話に同居した入力側捏造（「探す」画面フィルタ調査セッション／K型の再来・検知器起点）（確度: 高）
場所: `~/.claude/projects/-mnt-c-Users-qingj-Desktop-project-novel-reader-andloid/370800c1-2431-4028-af49-d164fa7e27ac.jsonl`
スナップショット: `~/.claude/hallucination-archive/370800c1-2431-4028-af49-d164fa7e27ac-20260716-150310.jsonl`（101行・**行番号はスナップショット基準＝live と同一内容**）
（**WSL/Linux 側セッション**。`~/wt/api-lab-ai` worktree・claude-opus-4-8。**事象G と同一セッション・同一レコード**＝1セッションに複数クラスの幻覚が同居した初例。D4 較正の slug 全走査〔2026-07-11 増補6 開発時〕で発見→handover「人間確認待ち」→2026-07-16 人間確認で昇格）

| 行 | uuid |
|---|---|
| L78 | `f7f52d38-ed59-4508-8d3b-a40a22a018e5` |

※ L78 の assistant text 地の文の中に「user[Request interrupted for tool use]」というロールマーカーと、続く幻のユーザー発話「会話率　複数選択にしたときに、あまり選択されないなら実装しないほうがいいなど、そういう挙動の複数選択にする意味についても言及してほしい」を自己生成（K と同型の入力側捏造）。幻発話の内容（会話率/複数選択にする意味）は**セッション内の実 human 入力に0件**（2026-07-16 transcript 直読で再突合）。本物の割込は type=user の独立レコード（L30/L86/L98）で記録されるのと対照的に、これは type=assistant の text ブロック内＝構造的に捏造。**G が同じ L78 を「Bash ブロックは回避し」（環境コンファビュレーション）で登録済み**＝同一発話が2クラスの幻覚を同居させたためアンカー行は重複するが、幻覚クラスが異なるので独立レター（G は環境への虚偽因果投影・P はユーザー入力の捏造）。**前兆**: 直前 thinking signature L77=4,252＜セッション中央値 8,140＝暴走 thinking 前兆なし（K と同様。G の暴走 thinking L55=139,860/L68=80,992 は先行事象で L78 直前には収束済み）。**検知（実測・2026-07-16 スナップショット走査）**: active 2件＝**A3 `fabricated_harness_block`（conf 0.90）＋ D4 `phantom_turn_role_marker`（conf 0.85）の二重検知**（増補6(4) の A3 変種拡張＋D4 新設の狙いどおり。K が「発見するが active 化できず」だった較正穴の解消を実データで実証した初例）。

---

## Q. 幻の出力待ち参照＝実行ゼロの新ターンで「出力が返ってこない」と先行実行を前提化（レーン統合検証セッション／N の L44 と構造同型・検知器起点）（確度: 高）
場所: `~/.claude/projects/-mnt-c-Users-qingj-Desktop-project-novel-reader-andloid/22ed3266-05f9-4c76-87d8-0230eccaface.jsonl`
スナップショット: `~/.claude/hallucination-archive/22ed3266-05f9-4c76-87d8-0230eccaface-20260716-150310.jsonl`（381行・**行番号はスナップショット基準＝live と同一内容**）
（**WSL/Linux 側セッション**。canonical・main・claude-opus-4-8。ブランチ未統合数の確認作業中。Tier E3〔増補7〕較正の全走査〔2026-07-16〕で発見→同日人間確認で昇格＝**E3 が設計狙いどおり N 型の単発再発を捕捉した初例**）

| 行 | uuid |
|---|---|
| L39 | `a2b3229b-d20f-494a-9c6e-8ce1fb36a4d6` |

※ 連鎖: L29 Bash `git rev-list --count main..verify/device-20260711`（実行）→ L30 tool_result「verify/device ahead of main: 0 commits」＝**出力は返却済み** → L34 user 割込 `[Request interrupted by user]`（中断された実行中コマンドは不存在＝L30〜L34 間に tool_use ゼロ・attachment/queue-operation のみ）→ L36 実ユーザー入力「どう？」→ **L39「出力が返ってこないので、単純なコマンドで確認します。」**＝新ターンで実行ゼロのまま、存在しない「返ってこない出力」を前提化（N の L44 と同構造の幻の先行実行参照。N と違い後続の偽成功報告への連鎖は無く単発で、直後 L40 の Bash `git merge-base` から実実行に復帰）。**前兆**: 直前 thinking signature L38=7,416＝セッション中央値 1,960 の約3.8倍＝N と同帯の中程度前兆（G〜J の 5〜30倍帯には届かない）。**検知（実測）**: **Tier E3 `phantom_prior_execution_reference`（conf 0.55・増補7）が live（canonical）走査で active**＝検知器起点の発見。アーカイブのスナップショット走査では**サブエージェント JSONL の解決が transcript パス相対**のため `subagent_unresolved` で降格（`--include-suppressed` で可視・照合自体は成立）＝スナップショット単独で再現する場合は降格込みで見ること。
