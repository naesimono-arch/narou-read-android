...
> **本文中の `apks_decompiled/`・`_recon/` 配下のファイル（`*.java`・`*.xml`・recon メモ等）は調査時のローカル作業成果でありリポジトリには含まれない**
> （デコンパイル成果物は非収蔵＝05/06 と同じ扱い）。参照は「どこを見て確定したか」の出典表示であって、このリポジトリ内のパスではない。

関連: [03 API機能の検討](03-api-feature-analysis.md) — 本解析が参照材料として支える、自作アプリへのAPI機能追加の検討本体。
