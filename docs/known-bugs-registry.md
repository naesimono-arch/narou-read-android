...
| `[!] なし` | `a11y-contrast-below-aa` | 文字・アイコンが薄くて読めない（4.5:1 未満） | 意匠の淡色をそのまま情報テキストへ使う。コントラストを測る機械が無い | ルビ3色の AA 化／発見系メタ6箇所を情報テキスト役割トークンへ／履歴チップの未ピンアイコン可視化 | なし | `docs/decisions/0014-design-principles-and-source-layers.md` |
...
| 状態 | ID | 症状 | 機序・バグ型 | 修正の所在（コミット件名の要約） | 検知手段 | 関連 knowledge |
|---|---|---|---|---|---|---|
| `[!] なし` | `hook-output-not-delivered` | フックは動いているのに通告がモデルに一切届かない（主機能が無音で不達） | イベントごとに届く出力先が違う（stdout / stderr / additionalContext）のを取り違える。**7回発生** | schema・lint フックの additionalContext 化／センチネル状態遷移通知／コミット粒度チェック／remind_commit_plan の stderr 化／remind_task_diary の注入化／mark_kotlin_tests_passed の不達／センチネルが Bash 出力形式を読めず未更新 | なし（`check_hook_smoke` は構文と自己テストのみで出力経路を見ない） | `docs/decisions/0008-no-hook-dispatcher.md` |
| `[!] なし` | `stale-check-false-positive` | 機械チェックが偽陽性を出し、報告が信用されなくなる | 検査対象の除外条件を実態に追随させていない | hooks_common.py の死hook 誤検知を除外／test_*.py・MEMORY.md の抑制／block_destructive_migration の対象を .kt 限定 | なし（`.claude/skills/stale-check/check_machine.py` 自身に自己テストが無い） | — |
| `[!] なし` | `checker-fail-open-skip` | 検査器が「SKIP」を黙って飲んで全通過する | 対象が見つからない＝合格、という fail-open 設計 | check_design_tokens の SKIP 内訳を列挙・ベースライン超過で exit 1 | なし（`tools/check_design_tokens.py` は自動起動もされない） | `docs/decisions/0018-derived-mock-drift-optin-sync-check.md` |
...
| `[!] 知見のみ` | `removed-hook-leaves-dead-consumer` | 撤去したフックの生成物に依存する判定が恒久 dead 化し、**テストは緑のまま13日間死ぬ** | 撤去コミットが「撤去する側」しか触らず、参照する側が残る | 恒久 dead だったセンチネル照合を退役／捏造検知のセンチネル参照を Kotlin 単一化 | 知見のみ（CLAUDE.md「フックの撤去は参照する側まで1セット」） | `docs/decisions/0006-detect-fabricated-execution-static-analysis.md` |
| `[!] 知見のみ` | `delegation-deletes-out-of-scope` | 委譲した結果、指示範囲外の機能が黙って消える | 生成委譲の diff を追加行だけ見て承認する | 条件シートの「今月」「先月」チップを復元（委譲バッチのスコープ外削除の退行） | 知見のみ（CLAUDE.md「削除行込み diff 全量レビュー」） | — |
