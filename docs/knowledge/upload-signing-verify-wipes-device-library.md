...
- **改善候補（handover 登録済み）**: 本文欠落検出時の再取込導線。⚠️ **当初あてにした「v20 の `books.sourceUri`＋永続 READ 権限が土台になる」という見立ては 2026-07-29 の実機実測で否定された**——uninstall は永続 URI 権限も道連れにするため、この機序では自動再取込が構造的に0件になる（→ `auto-backup-does-not-restore-uri-permissions.md`）。復旧は中身のハッシュ照合＋フォルダ権限の永続化で設計する。
