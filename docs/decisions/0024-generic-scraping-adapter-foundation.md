...
## 追記（2026-07-23・規約グレー勢のまとめ裁定＝ユーザー確定）

**裁定**: ハーメルン＝**保留続行**（pendingHosts 残置）／アルファポリス・Pixiv・野いちご・ベリーズカフェ＝**NG**
（blockedHosts へ移動＝公式送り）／**G2 ヒューリスティックは今はやらない**（「キリがない」＝対応面の際限拡大を止める判断）。

**⚠️ この NG は「グレー領域の保守裁定」であって、明白な違法・規約明文違反の認定ではない**（後日振り返る前提で明確に記す）:
- 4サイトとも自動取得・スクレイピングを**名指しで禁じる明文は確認されていない**。根拠は規約の**包括条項**
  （アルファポリス10条3項・野いちご/ベリーズカフェ第5条の「複製・公開・送信…翻案・転載・再利用の一括禁止」）で、
  端末内保存＝「複製」に文言上該当するという**保守側の読み**。私的複製（著作権法30条）との関係は法解釈依存。
- Pixiv は規約に加えて **R-18/会員ページのログイン必須＝アプリ内 Cookie 認証設計が前提**という技術コストの複合判断。
- **再訪の道**: 各社の企業窓口への事前照会・規約改定・公式APIの登場のいずれかで再裁定可。解放手順は
