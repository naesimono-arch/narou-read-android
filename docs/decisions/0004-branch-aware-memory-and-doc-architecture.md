...
3. **`guard_commit_branch.py`（PreToolUse / matcher:"Bash"）** — `main` への直接 `git commit` をコマンド境界に限定した正規表現で検知し `exit 2` でブロックする。**検査のみ**でセンチネルは消費しない。改行区切り・グローバルオプション付き（`git -C/-c … commit`）も検知し、クォート内の単なる言及は誤ブロックしない。**2026-07-06 拡張**: コミットを生成する `merge`/`rebase`/`cherry-pick`（`--abort`/`--quit`/`--no-commit` は除外）も検知対象に追加（競合しない main 直マージ・`merge --continue` が "commit" トークンを含まず素通りした実地事象への対処）。検知正規表現は `check_commit_granularity.py` 等とも同一定義で、`test_hooks.py` が全コピーの一致を回帰固定する。**2026-07-07 拡張**: `git switch main && git merge …` のようにコマンド内で保護ブランチへ `switch`/`checkout` してから commit/merge する複合コマンドも検知対象に追加。**なぜ**: PreToolUse 時点の `git branch --show-current` は切替前の（非保護）ブランチを返すため、カレントブランチだけの判定では素通りしていた（実地で 1 回踏んだ）。対処として `SWITCH_TO_PROTECTED_RE`（switch/checkout の移動先が保護ブランチかを完全トークン一致で判定・`checkout … -- <path>` のファイル復元と `main~1`/`feature/main` 等は除外）を追加し、実効ブランチを補完する。この正規表現は guard 固有で他フックに複製しない（`test_hooks.py` の `SwitchToProtectedRegex` が回帰固定）。**残る限界**: PowerShell 経由・難読化（`cd` + 相対や変数展開で switch/checkout の形を崩す等）は従来どおり対象外＝ソフト境界（下記 Consequences を参照）。
