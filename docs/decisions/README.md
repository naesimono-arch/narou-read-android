...
| [0027](0027-release-scope-feature-gate.md) | 初回公開のスコープ機能ゲート＝スキン機能ごと隠す（公開は明快K 単独・適用点は「きせかえ行／wardrobe ルート／skinFromName の復元」の3つだけ・prefs は消さず読み替え・判定はフラグ引数の純粋関数＝課金導入時に中身だけ差し替え。enum 削除/flavor 分離・BuildConfig 直書き・release の隠し入口・内部検証専用 buildType 却下。2026-07-29 Accepted・実装未着手） | — |
