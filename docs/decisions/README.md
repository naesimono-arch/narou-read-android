...
| [0020](0020-vertical-reading-scroll-custom-typesetting.md) | 縦書きモード＝右→左 連続横スクロール × 自前Compose組版（公式 text-vertical 成熟までのつなぎ・VerticalTypesetter interface で隔離。ページめくり骨格B・WebView writing-mode・公式待ち 却下） | — |
...
| [0027](0027-release-scope-feature-gate.md) | 初回公開のスコープ機能ゲート＝スキン機能ごと隠す（公開は明快K 単独・適用点は「きせかえ行／wardrobe ルート／skinFromName の復元」の3つだけ・prefs は消さず読み替え・判定はフラグ引数の純粋関数＝課金導入時に中身だけ差し替え。enum 削除/flavor 分離・BuildConfig 直書き・release の隠し入口・内部検証専用 buildType 却下。2026-07-29 Accepted・実装未着手）／**追記 2026-07-30: 隠す対象の保守スコープも凍結**＝M/P/J（D/C）の意匠差分・未完了項目は初回公開まで新規着手しない・既存実装は削除せず解禁は課金アップデート便と同時。対象外は〈共通実装の波及（route層・共有Composable）〉〈exhaustive when を通す最小翻訳〉〈クラッシュ/データ破損〉 | — |
| [0028](0028-ledger-split-by-blocking-party.md) | やること台帳を「待ちの種類」で二分＝`handover.md`（Claude が今すぐ動ける）／`awaiting-human.md`（人間の目視・裁定・外部手続き待ち・各項目に「何を見れば決まるか」必須・目視は実機ツアー順・裁定は放置した場合の帰結つき）。迷ったら handover 側／完了は消す運用は不変（0017 決定2 の細分。STATUS へ寄せる・handover 内に節を作る・主題別分割・完了刈りのみ・plans のチェックリストで代用 却下） | — |
| [0029](0029-dependency-ceiling-is-kotlin-1922.md) | 依存バンプの天井は compileSdk ではなく **Kotlin 1.9.22**（Roborazzi 1.30.1 が 1.9.22 ビルドである連鎖）。`mv=2.1.0` は機械的に死・`mv=2.0.0` 群は KGP 2.0+ 必須＝**Kotlin 2.x ＋ compose compiler plugin ＋ Roborazzi を1便**にするしかない（個別バンプの「別便で」据え置きが5件溜まった反省） | — |
| [0030](0030-no-l3-knowledge-injection-hook.md) | 再発防止 L3（knowledge の Edit/Write 時自動注入フック）は**不採用**＝機械検知への変換が上位互換（注入は「読まれること」に賭ける＝`知見のみ` と同じ弱点・効くのは無防備21件中6件のみ・常駐フックの保守リスク。今後「知見のみ」への既定の一手は triggers 付与でなく検知への変換） | — |
