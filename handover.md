...
- **[目次/本文] スクロール・アニメーションの最適化**（2026-07-15 実機フィードバック・**症状未言語化**）: 目次画面・読書本文まわりの挙動/アニメーションに違和感あり。まず作法違反の機械監査（`chrisbanes-skills:compose-animations`/`compose-recomposition-performance`/`compose-state-deferred-reads`）で洗い出し、それでも残る体感差は画面・操作を指差してもらってから計測 or 比較試作で切り分け（決め打ち修正はしない＝memory `feedback-perf-jank-is-real-signal`）。
