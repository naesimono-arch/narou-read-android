...
- **[2026-07-16・増補7 副産物] 22ed3266 の台帳レター事象化（人間確認待ち）**: Tier E3（先行実行フレーミング＝L44/L55 型・増補7 で実装済み）較正の全走査で発見した候補真陽性＝session `22ed3266-05f9-4c76-87d8-0230eccaface` order37「出力が返ってこないので、単純なコマンドで確認します」（先行 Bash は結果返却済み→ユーザー割込→新ターンで幻の出力待ち参照＝L44 と構造同型）。370800c1 と同じ検知器起点の発見経路。人間確認が取れたら台帳の追記手順（事後検証モード）でレター事象化。
