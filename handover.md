...
- **[DB・要判断] 次に実機へ入れるレーンの専有を決める（v15/v16 並走中）**（2026-07-11・db-migration スキルの衝突クラス）: 実機は v14。コード側は `feat/episode-nav`＝v15・`ui/vertical-pdf-import`＝v16（14→15 複製でパス繋ぎ済み）と採番が並走しており、**双方が実機投入すると合流時にエンティティ差で identity hash が再衝突**する（前例 v9→v10 の no-op 再スタンプ＝task_diary #39 追補）。実機を握るレーンを片方に決めてから投入すること。読書状態フィルタ（v16）の実機目視もこの判断待ち。
