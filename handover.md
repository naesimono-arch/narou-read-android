...
- **[意匠・演出] 了スタンプ押下アニメ**（2026-07-12 ユーザー要望）: 本が読了になった瞬間（`reachedEnd` が false→true）に、書影左下の朱印「了」を**印鑑を押すように**出す演出（スケール punch-in＋透過フェード等）。現状は即時表示。読了という達成の手触りを足す狙い。実装時は Motion トークン（`theme/Motion.kt`）経由・スタンプは1回きり（再表示のたびに再生しない＝初回点灯のみ）・`BookCard.kt` の `isFinished` バッジ（左下 seal）に付与。装飾アニメ禁止則（ADR0014 §C 静謐）との整合＝「状態変化の伝達」に該当する演出として正当化できるか要検討。
