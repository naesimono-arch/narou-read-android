...
- **[UIラウンド 2026-07-24] 残タスク**（実装・正本昇格・ゲートは完了＝STATUS 参照。裁定の一次情報＝
  `.claude/plans/ui-density-swipe-round-2026-07-24.md`）:
  - **実機目視**: K本棚（2列改A/圧縮S/D改破線/影）・タブスワイプ（3ボタンナビ/ジェスチャナビ両方・端の戻ると共存）・
    気分ページャ（日替わり注記・ドット）・ダークテーマの**影の視認**（沈むなら elevation/scrim を実機で詰める＝暫定2dp）。
  - 破線間隔 4/3dp・紙地沈め alpha0.05 も同便で目視調整。
  - 非Kスキンの気分は CLASSIC 固定＝ページャ化・日替わり適用は各スキン意匠の別ラウンド（J の扉 glyph が P1 前提）。
