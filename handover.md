...
- **[本棚] 栞ランダム抽選の新規取込での実機一発確認**（2026-07-16 実装完了の残り火）: v19 実装・実機 migration 検証済み（既存蔵書 NULL→フォールバック確認済み）。**新規取込時に真の乱数抽選値が DB へ焼かれ書影に反映される**ことだけ実機未確認（単体テストでは担保済み・残7⑥の picker 取込計測と同便で `SELECT shioriTipIndex, shioriLenFrac` が非NULLになるのを見れば閉じる）。
