...
- **[機能②・要設計判断] WebView の戻り遡行で読書位置が巻き戻る**（2026-07-11 実機スモークで実測）: `onPageFinished` は WebView の goBack 遷移でも毎回発火するため、ep1→ep2→ep3 と読み進めた後に**戻る操作で履歴を遡って退出すると、記録が最後に表示したページ＝セッション先頭話（ep1）へ上書き**される（N1389MK・N9754MK の2作品で再現）。仕様「最後に開いた話 N」の文言には忠実だが、「続きから」の UX 期待（読み止めた最遠話）とはズレる。対処候補: 前進遷移のみ記録（履歴スタック深さ比較）／max(既存, 新値) 単調更新（読み返し意図と衝突する副作用あり）等。ADR 0012 の追補と合わせて判断。
...
- **実機の `web_reading_progress` に検証由来の2行が残存**（2026-07-11 機能②スモークが記録: N1389MK ep=1〔ユーザーの本棚 Web カード＝「続きから 第1話」表示になる〕・N9754MK ep=1〔本棚外・検索経由のみ〕）: 実データと同形式のため実害なしだが、ユーザーの実読書位置ではない。気になるなら該当2行の DELETE のみで消える（books/web_novels には触れない）。
