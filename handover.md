...
- **スキン候補の棚卸し完了→画面モック展開（進行中）→Compose量産（別セッション）（2026-07-17 判定確定）**:
  1. ~~候補棚卸し~~ **完了**: 旧A〜J全面チェック→A/B/E/F/G/H/I差し戻し（D同形の色違い・毒々しい配色）→新規7＋後継2をサブエージェント量産→デザインダイジェスト磨き込み→最終判定「qpm」＋J追加指示＝**実装対象 M 星図・P カートリッジ・Q 読書の庭・J ポータル・デッキ**。保留=L発車標/N文机/O短冊/R文/S映写室（`docs/design-candidates/skins/candidates/` に保全・凍結。ギャラリー=`_gallery-all.html`・生成=`tools/build_skin_gallery.py`）。K絵巻はテーマごと退役。旧A〜J原本は claude.ai/design `ui-n-phase0/` に保持。
  2. **進行中（このセッション）**: M/P/Q/J それぞれ「そのスキンのコンテキストに合った各画面モック」（読書・発見・目次・設定。本棚は確定済み）を専任エージェントで展開→目視ループ。**追加規律（2026-07-17 ユーザー指示）＝①D正本モックの機能要素と全数突合（省略・創作機能の禁止）②余白を能動的に使い情報量で殴らない**。生成規範=`.claude/plans/skin-design-digest-2026-07-17.md`（§A規範・§B検収）。⚠️ ADR 0021 の「スキン固有モックは本棚+読書の2画面・他画面はD構造へ写像」規約は本タスクで全画面固有モックへ拡張＝**ADR 0021 要追記**。
  3. **別セッション**: オーケストレーションの長期自律ループで M/P/Q（＋C構造層）の Compose 実装→実機検証。モック確定後に実行起動ブロック付きプランを残して引き継ぐ。
  ※ DesignSync は主セッション限定＝モック fetch/同期は委譲不可。目視関門は各ステップで人間。M/P/Q は構造ごと別物＝トークン層だけでなく画面構造の切替が要る（ADR 0021 の「I/J枠外」判定と同じ論点＝機構拡張の設計判断が Compose 化の前に必要）。
