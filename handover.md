...
- **[スキン・改善案バックログ（2026-07-17 M/P/J実装セッションで採取）]**:
  - **[実機検証2026-07-17の改善案・要判断]** ①P章扉のpixel話数を一段強調（モック追補要） ②テーマ選択肢の統一＝本棚⋮3択 vs 読書シート4択「システムに従う」の不一致（全スキン共通の既存差・どちらへ揃えるかユーザー判断） ③HI-SCORE1位金の微燐光パルス（P静止則 ADR0022§3 と衝突＝要裁定） ④J一覧の栞先端色を扉ambientに連動（モック追補要） ⑤M遠景視差のデバッグ用factor誇張手段（開発補助）。
  - **P/J一覧ビューの自スキン意匠化**: 現状はM先例に倣い一覧トグル先=D構造フォールバック（機能は全数担保）。P mock `.li`・J mock グリッド面には専用意匠があるため、実機目視で「一覧に切り替えた瞬間に世界観が切れる」と感じたら翻訳する（P目次/J本棚の検収報告に詳細）。
