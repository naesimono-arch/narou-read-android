...
- **[DB] `feat/episode-nav`（コード v15）は合流まで実機投入禁止**（2026-07-11 決定＝実機レーン専有は `ui/vertical-pdf-import` がユーザー承認で取得し v16 を実機に刻んだ・現況は STATUS 冒頭）: episode-nav を合流させるとエンティティ集合が変わり実機 v16 と identity hash が再衝突し得る → その場合は **v17 の no-op 再スタンプ**で解消（前例 v9→v10＝task_diary #39 追補）。
