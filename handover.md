...
- ~~**条件を調整シートの高速フリック「枠オーバーシュート」**（2026-07-08・未修正・不具合残置）~~ → **上流解消と判定**（2026-07-12・material3 1.3.1 sources jar 一次確認＝settle の既定アニメが 1.3.0-alpha02 で spring→tween 化されオーバーシュート不発生。「本命の解」実装は不要。詳細は **task_diary #51 増補**が正本）。「シートが全画面」観測は内容増による見え方でモック乖離→85%上限で追従済み（`da42089`）。**残タスク＝実機の高速フリック目視スモークのみ**（GREEN なら本行削除可）。
