...
- **[K残・軽] ADR 0021 追記**: 既定スキン D→K の切替と恒常ボトムナビ導入（裁定根拠＝plan `default-ui-clarity-K-2026-07-23.md`）を機構ADRへ1節追記。**＋K形伝播（2026-07-23）で恒常ナビ・設定画面が全スキン共通層へ昇格（MEIKAI_K 限定撤廃・単一テーマスキンは相の固定表示）も同節で**（裁定根拠＝plan `k-shape-propagation-2026-07-23.md`）。
