...
- **[目次/本文] モーション最適化 — 残りは P1/P2 のみ**（2026-07-15）: 確定差分は適用・**遷移は実機確認済みで slide push に統一**（ADR 0019・ルート遷移＋目次⇄本文を同じ向き/尺 250ms＝進む右→左/戻る左→右）／本文 stretch オーバースクロール無効化（D2・目次/本棚は据え置き）。競合解析＝`docs/reference/06-competitor-reading-motion.md`。**残**: **P1**＝章→章（話送り）は現状どおり瞬間で据え置き中＝要望が出たらスライド化（kakuyomu 400ms が実在目安・新規演出のため症状指差し待ち）／**P2〔主因確定・要方式選定〕**＝**画面遷移（slide push）が毎回軽くカクつく**（2026-07-15 報告）。debug/release 切り分け→仮説①（ShioriCover 描画/本文コンポーズ）狙い撃ち実験は棄却→**2026-07-16 Perfetto escalate 完了で主因確定＝遷移中に本棚 LazyVerticalGrid の measure/layout が走ること**（最悪フレーム 104.8ms の約半分が `AndroidOwner:onMeasure` 51ms〔内側に `BookshelfScreen.kt:733`〕・本棚を介さない目次⇄本文のみでは onMeasure 合計 1406→253ms/最悪 jank 67ms・ShioriCover は全体 3.3ms で無罪確定。副次＝テキスト measure＋遷移アニメ中 recompose〔NavHost/AnimatedContent〕の複合）。経緯・全数値・修正候補3案（本命＝遷移中はグリッドの measure/layout を凍結しアニメ完了後に実グリッドへ差替え）＝`.claude/plans/reading-transition-jank-measurement-2026-07-16.md` 末尾「Perfetto escalate」節。**次＝方式選定→実装→同条件で再計測**。
