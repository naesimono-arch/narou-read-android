...
- **Macrobenchmark 残フェーズ**（基盤＋コールド起動の実機初回計測まで完了＝median 252.9ms。実測・予算候補・ColorOS の壁の詳細＝`.claude/plans/macrobenchmark-kickoff-2026-07-17.md`＋`docs/knowledge/coloros-uiautomation-shell-pipe-eof-hang.md`）: ①起動予算 assert（候補 median≤350ms/max≤500ms・`-e enableBudgetAssert true` ゲート）＋ベンチ実行スクリプト化（SIGQUIT 除細動ループ同梱） ②10倍蔵書シーダー（src/benchmark ソースセット）＋本棚スクロール jank ③長時間章送り jank ④大PDF取込（TraceSectionMetric）。
