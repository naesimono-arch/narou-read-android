...
- **スキン候補モックの全面棚卸し→10種化→画面モック展開→自律ループ量産（2026-07-17 ユーザー指示・着せ替え外枠の次タスク）**:
  1. 現状の着せ替え候補 HTML モック（A〜J＝claude.ai/design `ui-n-phase0/` 保持分＋収蔵済み C/D）を**全面チェックし、1案ずつ OK/差し戻しを判定**。合計 **10種類**になるよう調整（差し戻し分の作り直し・不足分の新規起案を含む）。
  2. 採用各スキンについて「そのスキンのコンテキストに合った**各画面モック**」（本棚・読書・発見・目次・設定 等）を起こし、目視チェックのループを回す。
  3. 最後に**オーケストレーションの長期自律ループ**（サブエージェント編成）で各スキンの Compose 実装を一挙作成し、実機確認・検証まで通す。
  ※ DesignSync は主セッション限定＝モック fetch/同期は委譲不可。目視関門は各ステップで人間（PushNotification→目視OK→コミット）。
