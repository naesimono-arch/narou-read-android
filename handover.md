...
- **[目次/本文] モーション最適化 — 残りは P1/P2 のみ**（2026-07-15）: 確定差分は適用・**遷移は実機確認済みで slide push に統一**（ADR 0019・ルート遷移＋目次⇄本文を同じ向き/尺 250ms＝進む右→左/戻る左→右）／本文 stretch オーバースクロール無効化（D2・目次/本棚は据え置き）。競合解析＝`docs/reference/06-competitor-reading-motion.md`。**残**: **P1**＝章→章（話送り）は現状どおり瞬間で据え置き中＝要望が出たらスライド化（kakuyomu 400ms が実在目安・新規演出のため症状指差し待ち）／**P2〔発火・要調査〕**＝**実機で画面遷移（slide push）が毎回軽くカクつくと報告**（2026-07-15 実使用フィードバック・「コード上の目立つ問題は無い」前提）。**決め打ち修正禁止＝計測から**（memory `feedback-perf-jank-is-real-signal`）。**まず debug/release で切り分け**＝debug は Compose の遷移 jank が誇張される（R8未適用・composition デバッグオーバーヘッド。現APK＝`minifyEnabled false`）ため debug 特有の可能性が高い（`profileable` か release で再確認）。実要因の候補＝①遷移先の初回コンポジション/初回レイアウト（本棚/目次/本文の `LazyColumn`/`LazyVerticalGrid`）がアニメ第1フレームに同居しフレーム落ち ②遷移中に遷移先の重い読み込み（HTMLパース/目次/章数え）が並走。手順＝遷移中に `adb shell dumpsys gfxinfo com.novelreader` の jank%/フレーム時間・Perfetto trace・Layout Inspector の recomposition を採取→ボトルネック特定後に対策（遷移先コンテンツのアニメ後ロード・初回コンポジション軽量化等）。**D2 の端バウンド消えの実機目視は未取得**（低リスク・次に実機を触るとき確認）。
