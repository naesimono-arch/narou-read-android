...
- **[裁定済み 2026-07-20] サイト齟齬・規約＝あいまいは後でまとめ裁定／漏れは発見次第追加**: アルファポリス除外可否・
  ...
  **→ 注2 の土台は 2026-07-23 実装済み**（ScrapeHttpClient の per-host スロットル＝アダプタ宣言 `crawlDelayMs`＋グローバル床1s・429/503 Full Jitter バックオフ・Retry-After 尊重・403/404 即中止）＝アルファポリスは裁定が下りればアダプタ追加のみで着手可。
...
- **[裁定済み 2026-07-23] 汎用アダプタ方針＝GO／規約グレー勢のまとめ裁定＝引き続き保留**: ユーザー裁定
  ...
  **→ OK側の表駆動候補は暁で打ち止め。個人サイト系を拾う実質手段は G2 ヒューリスティック（要否は別裁定）か
  Wayback 専用処理のみ**＝次の裁定点。
