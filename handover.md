...
- **本棚・操作**: ①1画面あたりの表示冊数を増やす（本棚グリッドを詰めて表示＝「1ページ毎の本の量追加」・2026-07-20 解釈確定）／②**ブラウザ直行で追加した作品が長押し削除できない（バグ・要再現調査）**／③削除（複数選択）モードが「いま何のモードか」分かる状態表示／④長押し時の触覚フィードバック（振動 haptic＝**後回しでOK**とユーザー明言）。
...
- **[hooks・テスト宿題] `inject_subagent_briefing.py` の自動テスト未整備**（2026-07-19 実装時は手動検証のみ＝
  agent_type 各分岐・fail-open を echo JSON で確認済み）: `.claude/hooks/test_hooks.py` 系への追加を検討。
  対象＝分岐（実装系/調査系/対象外/空 agent_type）・壊れた stdin の fail-open・JSON 出力形式（hookEventName/additionalContext）。
  ゲートコマンド文字列が CLAUDE.md のビルド手順と乖離したときに気づける突合があるとなお良い。
...
- **[要設計・縦書きモードで本文上部の作品タイトル常時表示が縦書き本文と重なる]**（2026-07-19 ユーザー報告・別タスク指定）: 縦書きは文章が上端から流れるため、上部常時タイトルと常に衝突する。縦書き時の表示対応（非表示/位置替え/クローム連動等）を検討。/visual-language 接地とモック突合から。
