...
- **[目次/本文] スクロール・アニメーションの最適化**（2026-07-15 実機フィードバック・**症状未言語化**→**分析完了・適用待ち**）: 競合5アプリのデコンパイル解析は完了し `docs/reference/06-competitor-reading-motion.md` へ収蔵。自作実装との突合と適用案＝`.claude/plans/reading-motion-apply-2026-07-15.md`（2026-07-15）。**確定差分2点を先行適用してよい**（5社実測がエビデンス＝決め打ちでない）: **D1**=NavHost 遷移未指定で nav 2.7 既定の **700ms フェード**が全遷移に効いている（`MainActivity.kt:206`・Motion.kt 禁止則②抵触状態→250ms 程度をトークン新設して明示）／**D2**=読書本文の stretch オーバースクロールが既定 ON（各社は読書面で消す・目次/本棚は据え置きが業界一致→本文 LazyColumn のみ無効化）。D1→D2 実装→実機答え合わせ（APK 投入前に確認を取る）→**残る違和感のみ**画面・操作を指差してもらい、章送り演出(P1・kakuyomu 400ms が実在目安)か計測(P2)へ切り分け（決め打ち修正はしない＝memory `feedback-perf-jank-is-real-signal`）。
