...
- **[読書画面] 縦書き表示モードの実装 — 方針確定済み・実装待ち**（2026-07-16 方針合意）: 連続横スクロール（右→左）× 自前Compose組版（公式 text-vertical 成熟までのつなぎ・品質妥協なし）。裁定と根拠＝**ADR 0020**・フェーズ分割/スパイク＝`.claude/plans/vertical-reading-mode.md`・作業ブランチ＝`reading/vertical-mode`（worktree `~/wt/reading-vertical-mode`）。触感モック＝`docs/design-candidates/reading-vertical-scroll-D.html`（採用A）/`reading-vertical-paged-D.html`（不採用B・記録）。**同プランP2.5で本棚書影題字の正立崩れ（`ShioriCover.drawShioriTitle`＝「（）」「～」「ー」が回転せず死ぬほど見づらい・2026-07-16 ユーザー報告）も共有文字クラス部品で修正する**。
