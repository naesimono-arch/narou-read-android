...
- **⚠️ C 夜行の欠落モック初稿は方向不一致で差し戻し（2026-07-18）**: 発見/目次/設定＋一覧2案のドラフトを生成・提示したが「全然違う」（詳細未聴取のまま「今は既存を詰める方向で」の指示）。ドラフトは `candidates/c-draft/`・`candidates/list-C/` に未追跡のまま退避。**再着手時はまずユーザーの夜行イメージの言語化（何が違ったか）から始める**こと（憲法2枚＝bookshelf-C/reading-C の読み込みだけでは不十分と実証された）。
