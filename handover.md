...
- **09 グリフ太さ/紙質感/色味の追微調整**【次セッションの再開点】: `BookCover.kt` の `saturation 0.38〜0.49` / `lightnessTop 0.46〜0.61` 起点。色味はキリがないため**上限2周のタイムボックス**で候補提示→実機スクショ→ユーザー選択、OKが出なければ現状維持で打ち切り（2026-07-02 単発修正バッチの Step 6 として計画済み・プラン=`~/.claude/plans/synthetic-roaming-parnas.md`）。
