...
- **[目次/本文] スクロール・アニメーションの最適化**（2026-07-15 実機フィードバック・**症状未言語化**→**D1/D2適用済み・実機答え合わせ待ち**）: 競合5アプリ解析＝`docs/reference/06-competitor-reading-motion.md`・突合と適用案＝`.claude/plans/reading-motion-apply-2026-07-15.md`（5社実測がエビデンス＝決め打ちでない）。**確定差分は両方適用済み**＝**D1**（NavHost 遷移フェードを Motion トークンで 700→250ms）／**D2**（本文 LazyColumn のみ stretch オーバースクロールを無効化・目次/本棚は据え置き）。**次: D1/D2 まとめて実機答え合わせ**（APK 投入前に確認を取る＝memory `feedback-ask-before-device-testing`。コード定数と実機描画は一致するとは限らない前提で体感確認とセットで扱う）→**残る違和感のみ**画面・操作を指差してもらい、章送り演出(P1・kakuyomu 400ms が実在目安)か計測(P2)へ切り分け（決め打ち修正はしない＝memory `feedback-perf-jank-is-real-signal`）。
