...
- **[K残] ユーザー実機目視**（本棚/さがす/設定/目次/読書/装いの間）→OK後コミット。**フィードバック第1便
  ...
  `docs/knowledge/compose-root-surface-content-color.md`）。**書影輪郭線だけは蔵書0冊のため実機未目視**
  （16:54 にアプリ内削除フローで2冊削除の痕跡＝お試し操作と推定）→ 本を再取込したら目視確認を。
...
- **[K残・軽] ADR 0021 追記**: 既定スキン D→K の切替と恒常ボトムナビ導入（裁定根拠＝plan `default-ui-clarity-K-2026-07-23.md`）を機構ADRへ1節追記。
...
- **本棚・操作**: ①表示冊数増＝**K の3列グリッドで回答済み**（D 自体の密度変更は K が既定になったため取り下げ・要望再燃時のみ）／②**ブラウザ直行で追加した作品が長押し削除できない→真因確定（2026-07-23 調査）・K で意匠障害は解消（可視⋮あり）＝P2として実装可**: 配線欠落でなく設計分岐＝Web作品（ShelfItem.Web/web_novels）は長押しで複数選択モードに入らず自カード⋮メニューが開くだけ（`WebBookCard.kt:89/:189`・削除は「本棚から外す」→`removeWebNovel` で機能自体は健在・全スキン同構造）。選択モードの入口/全選択/削除確定が Book 限定（`BookshelfScreen.kt:547-549/:1029/:1254`）。修正方向＝Webカードも複数選択削除へ統合。**実装は要裁定で停止（近似禁止ゲート発動）**: キー設計は確定（既存 `ShelfItem.key` の接頭辞方式 `"book:<id>"`/`"web:<ncode>"` 流用・listSaver 不変）だが、**D/J グリッドの Web カードは長押しが⋮メニューへの唯一の導線**（可視⋮無し＝mock 明記）のため、長押しを選択入りへ転用すると「縦書きPDFを取り込む」（他導線ゼロ）・進捗時「目次」が到達不能。裁定選択肢＝(A推奨) 非選択時のみ TopEnd に既存⋮ IconButton を出す最小合成解（mock「フラット・⋮無し」に反する意匠承認要） (B) grid の取り込む/目次導線を捨て discovery 再検索へ委ねる（機能格下げ） (C=A/Bどちらでも同伴) 削除確認文言の出し分け（現行「変換済みの本文データも削除されます」は Web に虚偽＝Web は失うもの無し・即復元可）。裁定後は list 系3スキン＋VM/キー/テストは意匠非依存で即着手可／③削除（複数選択）モードが「いま何のモードか」分かる状態表示／④長押し時の触覚フィードバック（振動 haptic＝**後回しでOK**とユーザー明言）。
