...
- **[要設計・「戻る」操作の原則統一＝常に一つ前の階層へ]**（2026-07-19 ユーザー裁定・別タスク指定）: 横スワイプ戻る／左上戻るボタンとも、移動履歴（目次内の作品横移動等）を逆走せず**「必ず一つ前の階層に戻る」**へ統一する。現状の不一致の実例＝本棚→本文と遷移した際、横スワイプは本棚へ・左上ボタンは目次へ戻り挙動が割れている。ReadingBackStack の規約（既出画面へのpopUpTo/横移動は置換）との整合設計から着手。
