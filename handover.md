...
- **[高負荷モード横展開（ADR 0023・2026-07-19 ユーザー裁定）]**: 各UI/スキンの画面へ「高負荷モード（美しさ特化・負荷許容）」を順次追加していく長期方針。順序＝星図Mで軽く作成→知見を和モダンDへ→以降各画面。深追いしない。事前準備は実機時の2点のみ: ①実機APIレベル確認（AGSL可否・非対応はフォールバック）②現行スカイの jank% ベースライン採取（高負荷ON/OFF比較用）。進行中の一次情報＝`.claude/plans/richness-expansion-round-2026-07-19.md`。
