 ...
 * MIGRATION_7_8 / 8_9 / 9_10 / 10_11 / 11_12 / 12_13 / 13_14 / 14_15（`internal` 宣言＝同一モジュール・同一パッケージの androidTest から可視）。
 ...
 *     その層は `freshInstallAtV15_passesIdentityHashCheck`（実エンティティの hash 照合）が別角度で補う。
 *
 * **対象範囲を 7→15 に絞る理由**: identity hash 衝突を実測した版（v8/v9/v10）と最新の v11（F-G 恒久策＝
 * books.contentSha256 追加）、v12（web_novels テーブル新設）、v13（new_episode_marks テーブル新設）、v14（labels / book_labels 新設）、および v15（web_reading_progress 新設）がこの区間に集中する。MIGRATION_3_4〜6_7 も AppDatabase に実装は在るが
...
class MigrationTest {
    ...
    /**
     * 7→8→9→10→11→12→13→14→15 の全パスを1段ずつ適用し、各段で結果スキーマが checked-in JSON と一致するか検証する。
     *
     * **何を捕まえるか**: Migration の DDL が Room がエンティティから期待するスキーマ（8/9/10/11/12/13/14/15.json）と食い違う退行。
     * 食い違えば実機では起動時 schema validation で即クラッシュする——それを1段ごとに前倒しで検出する。
     */
    @Test
    fun migrate7to15_validatesSchemaAtEachStep() {
    ...
    fun migrate7to15_preservesExistingRows() {
        ...
        // 全チェーンを一括適用。最終 v15 のみ検証すればよく、15.json は8テーブル全部を含むので validateDroppedTables=true。
        val db = helper.runMigrationsAndValidate(
            TEST_DB_DATA, 15, true,
    ...
    /**
     * 最新版 v15 の「フレッシュインストール」が identity hash 検証を通ることを検証する。
     *
     * **何を捕まえるか**: エンティティ（BookEntity/ProgressEntity/PendingJobEntity/WebNovelEntity/NewEpisodeMarkEntity/LabelEntity/BookLabelEntity/WebReadingProgressEntity や @Database version）を変えたのに
     * schemas/15.json の再生成 or version 上げを忘れた退行。この乖離は実機の**起動即クラッシュ**の正体で、
     * 本プロジェクトが2回踏んだ事象（task_diary #39）そのもの。
     *
     * **機序**: ①15.json（checked-in の最新スキーマ）から v15 DB を作り、その identityHash を room_master_table に刻む。
     * ②実アプリの Room で同じファイルを開くと、Room がコンパイル時にエンティティから算出した identity hash を
     * ①で刻まれた 15.json 由来 hash と照合する（RoomOpenHelper.checkIdentity）。両者が乖離していれば
     * IllegalStateException で落ちる＝テストが赤くなり、実機投入前に検出できる。
     * （version 15 == 現行のため migration は走らず、純粋な hash 照合だけが起こる）
     */
    @Test
    fun freshInstallAtV15_passesIdentityHashCheck() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        context.deleteDatabase(TEST_DB_FRESH) // 前回の残骸を掃除して決定的にする（createDatabase も削除するが二重の安全）

        // ① 15.json から v15 DB を作り、15.json の identityHash を刻む。
        helper.createDatabase(TEST_DB_FRESH, 15).close()
        ...
        try {
            ...
            assertTrue(
                "v15 スキーマで Room が開けない＝実エンティティと 15.json の identity hash 不一致",
