...
// ============================================================
// ShioriTips — 栞書影の先端ワンポイント意匠データ（SHIORI_TIPS 174種）と、その描画語彙
// （ShioriTip typealias・strokeR/deg/starPath ヘルパー）。
//
// なぜ ShioriCover.kt から分離したか: 意匠データの羅列が約1560行に達し、描画ロジック
// （棒・題字・スケール補正）が同一ファイル内で埋没していたため。ShioriCover.kt からの
// 純移動（名前・値・可視性・順序は不変）であり、振る舞いは一切変わらない（2026-07-27 分割）。
// 意匠の増補手順は .claude/skills/shiori-tips が正本（「配列に1つ足すだけ」の拡張設計も不変）。
// ============================================================
